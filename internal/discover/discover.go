	...
	for i := 0; i < opts.Workers; i++ {
		...
		go func() {
			...
			for w := range tasks {
				...
				if err != nil {
					mu.Lock()
					results = append(results, Result{Path: w, Error: err.Error()})
					mu.Unlock()
					continue
				}
				...
				req.Header.Set("User-Agent", "MEB-Discover/1.0")
				...
				if err != nil {
					mu.Lock()
					results = append(results, Result{Path: w, Error: err.Error(), Duration: dur})
					mu.Unlock()
					if log != nil {
				...
				mu.Lock()
				results = append(results, Result{Path: w, StatusCode: resp.StatusCode, Length: length, Duration: dur})
				mu.Unlock()
			}
		}()
	}
