> Source: https://portswigger.net/burp/documentation/desktop/tools/infiltrator
...
**Read time: ** 7 Minutes
 ...
 Burp Infiltrator is a tool for instrumenting target web applications in order to facilitate testing using [Burp Scanner](https://portswigger.net/burp/documentation/scanner). Burp Infiltrator modifies the target application so that Burp can detect cases where its input is passed to potentially unsafe APIs on the server side.
...
1.
 When the application calls a potentially unsafe API, the instrumentation hook inspects the relevant parameters to the API. Any Burp payloads containing [Burp Collaborator](https://portswigger.net/burp/documentation/collaborator) domains are fingerprinted based on their unique structure.
...
1.
 Optionally, based on [configuration options](https://portswigger.net/burp/documentation/desktop/tools/infiltrator#configuration-options), the instrumentation hook makes an HTTP/S request to the mutated Burp Collaborator domain, including the full value of the relevant parameter and the application call stack.
...
![Burp Infiltrator](https://portswigger.net/burp/documentation/desktop/images/infiltrator/infiltrator-1.svg)
...
#### Note
 ...
 Before installing and running Burp Infiltrator, ensure that the application is not currently running, as this may prevent the bytecode on disk from being modified.


![Export the Burp Infiltrator installer](https://portswigger.net/burp/documentation/desktop/images/infiltrator/using_burp_infiltrator_2.jpg)
...
![Copy the Burp Infiltrator installer onto a machine containing the application bytecode](https://portswigger.net/burp/documentation/desktop/images/infiltrator/using_burp_infiltrator_3.jpg)
 ...
 Run Burp Infiltrator from the command line. For example, for Java applications, enter `java -jar burp_infiltrator_java.jar`.


 By default, the Burp Infiltrator installer runs interactively, and asks a series of questions during installation. Alternatively, you can run it [non-interactively](https://portswigger.net/burp/documentation/desktop/tools/infiltrator#non-interactive-installation).


![Run Burp Infiltrator from the command line](https://portswigger.net/burp/documentation/desktop/images/infiltrator/using_burp_infiltrator_4.jpg)
...
![Burp Infiltrator patching the bytecode](https://portswigger.net/burp/documentation/desktop/images/infiltrator/using_burp_infiltrator_7.jpg)

 Finally, [perform a scan](https://portswigger.net/burp/vulnerability-scanner) of the application.
...
![Burp Infiltrator scan the target application](https://portswigger.net/burp/documentation/desktop/images/infiltrator/using_burp_infiltrator_6.jpg)
...
### Non-interactive installation
 ...
 You can install Burp Infiltrator non-interactively. This supports various use cases. For example, a CI pipeline could automatically deploy an application build to a staging server, install the Burp Infiltrator instrumentation, and perform a scan using [Burp Scanner](https://portswigger.net/burp/documentation/scanner).
 ...
 Before installing Burp Infiltrator non-interactively, you should use the above command to view the details of the default values for [configuration options](https://portswigger.net/burp/documentation/desktop/tools/infiltrator#configuration-options) that will be used unless overridden, and ensure that the correct options will be used based on your requirements.


#### Note

 By running the Burp Infiltrator installer in non-interactive mode, you are deemed to have read and agreed to all warnings and disclaimers that are displayed during interactive installation.


## Configuration options

 The following configuration options can be specified during the [installation](https://portswigger.net/burp/documentation/desktop/tools/infiltrator#installing-burp-infiltrator) of Burp Infiltrator, either interactively or using command line options during [non-interactive](https://portswigger.net/burp/documentation/desktop/tools/infiltrator#non-interactive-installation) installation:
...
-
 Whether to allow communication with the Collaborator server over unencrypted HTTP. Using HTTPS reduces the risk of unauthorized disclosure of transmitted information. However, depending on the technologies available within the instrumented application, and the capabilities of the configured Collaborator server, it might sometimes be necessary to use unencrypted communications. Note that when HTTPS is used, Burp Infiltrator makes a best effort to use the latest TLS cipher that is available. On some target systems, particularly those using older technologies, the latest available cipher might be vulnerable to known attacks by a suitably located adversary.

-
 Whether to allow communication only to specified Burp Collaborator servers. During scanning by [Burp Scanner](https://portswigger.net/burp/documentation/scanner), payloads will contain subdomains of the Burp Collaborator server that Burp is configured to use. If you know that Burp scans will be performed using a specific Collaborator server, you can configure Burp Infiltrator to allow communication only to that server.
